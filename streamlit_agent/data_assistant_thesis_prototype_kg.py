...
st.caption("Version B: Knowledge graph-supported assistant")
st.info(
    "This version uses a knowledge graph to show connections between business "
    "concepts, database tables, columns, and joins."
)
...
- For returned-order count plus sales questions, do NOT use COUNT(*) after joining to Orders. Use COUNT(DISTINCT r."Order ID") and SUM(o."Sales").
...
def handle_explanation_click(interaction_id, explanation_type="written"):
...
def display_kg_explanation_controls(interaction_id, content, animation_html, show_all=False):
    ...
    show_written = show_all or st.session_state.get(
        f"written_explanation_{interaction_id}", False
    )

    with st.container(key=f"explanation_controls_{interaction_id}"):
        button_columns = st.columns(2) if animation_html else [st.container()]
        if animation_html and not show_animation:
            with button_columns[0]:
                st.button(
                    "Animated reasoning path",
                    key=f"animation_button_{interaction_id}",
                    on_click=lambda id=interaction_id: handle_explanation_click(
                        id, "animation"
                    ),
        ...
        if not show_written:
            with button_columns[-1]:
                st.button(
                    "Written explanation",
                    key=f"written_button_{interaction_id}",
                    on_click=lambda id=interaction_id: handle_explanation_click(
                        id, "written"
                    ),
                    use_container_width=True,
    ...
    if animation_html and show_animation:
        with st.expander("Animated reasoning path", expanded=True):
...
"""
    ...
    with st.spinner(text="Generating explanation..."):
        ...
        sql_explanation = explain_intermediate_steps(prettified_inter_steps)
